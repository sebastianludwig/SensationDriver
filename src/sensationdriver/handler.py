    ...
    class Vibration(pipeline.Element):
        ...
        def _init_actors(self, config_path):
            ...
            for region in config['vibration']['regions']:
                ...
                try:
                    region_index = sensationprotocol.Sensation.Region.Value(region['name'])
                except ValueError:
        ...
        def _process(self, sensation):
            if sensation.region in self.actors and sensation.actor_index in self.actors[sensation.region]:
                ...
                self.logger.debug("No actor configured with index %d in region %s", sensation.actor_index, sensationprotocol.Sensation.Region.Name(sensation.region))
